...
output_path ="D:/pywork/icassp2022-depression-main/DepressionCollected/Classification/output"
...
# audio_features = np.squeeze(np.load(os.path.join(prefix, 'DAICCode/Features/DAIC_ausamples_clf.npz'))['arr_0'], axis=2)
# audio_targets = np.load(os.path.join(prefix, 'DAICCode/Features/DAIC_aulabels_clf.npz'))['arr_0']
# fuse_features = [[audio_features[i], text_features[i]] for i in range(text_features.shape[0])]
# fuse_targets = text_targets
#
# fuse_dep_idxs = np.where(text_targets == 1)[0]
...
log_path = os.path.join(output_path,'log_attn1.txt')
...
def train(epoch, train_idxs):
    ...
    for i in range(0, len(X_train), config['batch_size']):
        ...
        loss = criterion(text_feature, audio_feature, torch.tensor(y).type(torch.LongTensor), model)
        # 后向传播调整参数
        loss.backward()
        # 根据梯度更新网络参数
        optimizer.step()
    ...
    return cur_loss/len(X_train),correct / len(X_train)

def evaluate(model, test_idxs, fold, train_idxs,epoch):
    model.eval()
    batch_idx = 1
    total_loss = 0
    ...
    output ="fold:{}\tepoch:{}\tacc:{}\tprecision:{}\trecall:{}\tf1_score:{}\n".format(fold,epoch,accuracy,precision,recall,f1_score)
    with open(log_path, "a+") as f:
        f.write(output + '\n')
        f.close()
    
    if max_f1 < f1_score and max_train_acc >= len(train_idxs)*0.85 and f1_score > 0.8:
        max_f1 = f1_score
        max_acc = accuracy
        # save(model, os.path.join(prefix, 'DAICCode/Model/Fuse/fuse_{:.2f}_{}'.format(max_f1, fold)))
        print('*'*64)
...
if __name__ == '__main__':
    idxs_paths = ['train_idxs_0.59_1.npy', 'train_idxs_0.60_2.npy', 'train_idxs_0.51_3.npy']
    text_model_paths = ['BiLSTM_128_0.74_1.pt', 'BiLSTM_128_0.82_2.pt', 'BiLSTM_128_0.67_3.pt']
    audio_model_paths = ['BiLSTM_gru_vlad256_256_0.67_1.pt', 'BiLSTM_gru_vlad256_256_0.72_2.pt', 'BiLSTM_gru_vlad256_256_0.65_3.pt']
    # idxs_paths = ['train_idxs_0.67_1.npy', 'train_idxs_0.66_2.npy', 'train_idxs_0.64_3.npy']
    # text_model_paths = ['BiLSTM_256_0.84_1.pt', 'BiLSTM_256_0.84_2.pt', 'BiLSTM_256_0.81_3.pt']
    # audio_model_paths = ['BiLSTM_gru_vlad256_512_0.67_1.pt', 'BiLSTM_gru_vlad256_512_0.66_2.pt','BiLSTM_gru_vlad256_512_0.64_3.pt']
    for fold in range(1, 4):
        # if fold != 2:
        #     continue
        train_idxs_tmp = np.load(os.path.join(prefix, 'Features/TextWhole/{}'.format(idxs_paths[fold-1])), allow_pickle=True)
        # train_idxs_tmp = np.load(os.path.join(prefix, 'DAICCode/Features/{}'.format(idxs_paths[fold-1])), allow_pickle=True)
        ...
        # text_lstm_model = torch.load(os.path.join(prefix, 'DAICCode/Model/{}'.format(text_model_paths[fold - 1])))
        # audio_lstm_model = torch.load(os.path.join(prefix, 'DAICCode/Model/{}'.format(audio_model_paths[fold - 1])))
        model_state_dict = {}
        ...
        for ep in range(1, config['epochs']+1):
            ...
            acc_v, precision = evaluate(model, test_idxs, fold, train_idxs,ep)
